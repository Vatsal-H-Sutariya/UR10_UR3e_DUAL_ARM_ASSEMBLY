...
class PP:
    def __init__(self, pose, g=0, h=0):
        self.pose = tuple(pose)  # (x, y, z) in meters
...
class RobotMover(Node):
    ...
    def heuristic(self, pose, goal):
        pos_dist = np.linalg.norm(np.array(pose[:3]) - np.array(goal[:3]))
        # Angular distance (approximate, using RPY differences)
        angle_dist = sum(abs(a - b) for a, b in zip(pose[3:], goal[3:]))
        return pos_dist + 0.1 * angle_dist  # Weight angular distance lower
    

        # 3D A* Path Planning\
    def a_star_3d(self, start_pose: list, goal_pose: list,   robot_type, grid_size,  pos_step=0.2,angle_step: float = 0.1):
        # Directions: 6-connectivity (up, down, left, right, forward, backward)
        

        directions = [
        # Position steps
        (pos_step, 0, 0, 0, 0, 0), (-pos_step, 0, 0, 0, 0, 0),
        (0, pos_step, 0, 0, 0, 0), (0, -pos_step, 0, 0, 0, 0),
        (0, 0, pos_step, 0, 0, 0), (0, 0, -pos_step, 0, 0, 0),
        # Orientation steps
        (0, 0, 0, angle_step, 0, 0), (0, 0, 0, -angle_step, 0, 0),
        (0, 0, 0, 0, angle_step, 0), (0, 0, 0, 0, -angle_step, 0),
        (0, 0, 0, 0, 0, angle_step), (0, 0, 0, 0, 0, -angle_step)
        ]
        ...
        # Create start node
        start_pose = [
            round(start_pose[0] / pos_step) * pos_step,
            round(start_pose[1] / pos_step) * pos_step,
            round(start_pose[2] / pos_step) * pos_step,
            round(start_pose[3] / angle_step) * angle_step,
            round(start_pose[4] / angle_step) * angle_step,
            round(start_pose[5] / angle_step) * angle_step
        ]
        goal_pose = [
            round(goal_pose[0] / pos_step) * pos_step,
            round(goal_pose[1] / pos_step) * pos_step,
            round(goal_pose[2] / pos_step) * pos_step,
            round(goal_pose[3] / angle_step) * angle_step,
            round(goal_pose[4] / angle_step) * angle_step,
            round(goal_pose[5] / angle_step) * angle_step
        ]
        start_node = PP(start_pose, g=0, h=self.heuristic(start_pose, goal_pose))
        ...
        while open_list:
            ...
            current_pos = np.array(current.pose)

            pos_diff = np.linalg.norm(current_pos[:3] - np.array(goal_pose[:3]))
            angle_diff = sum(abs(a - b) for a, b in zip(current_pos[3:], goal_pose[3:]))
            if pos_diff <= pos_step and angle_diff <= angle_step:
                path = []
                while current:
                    path.append(list(current.pose))
            ...
            for direction in directions:
                neighbor_pose = tuple(current_pos + direction)
                x, y, z = neighbor_pose[:3]
                roll, pitch, yaw = neighbor_pose[3:]
                
                # Check bounds and obstacles
                if (-2 <= x <= grid_size[0] and -2 <= y <= grid_size[1] and -2 <= z <= grid_size[2] and
                    -math.pi * 2 <= roll <= math.pi * 2 and -math.pi * 2 <= pitch <= math.pi * 2 and -math.pi * 2 <= yaw <= math.pi * 2 and
                    
                    not self.is_in_obstacle(neighbor_pose , robot_type) and neighbor_pose not in closed_set):
                     
                    # Calculate costs
                    g = current.g + (pos_step if any(d != 0 for d in direction[:3]) else angle_step)
                    h = self.heuristic(neighbor_pose, goal_pose)

                    # Check if neighbor is already in open list
                    neighbor = PP(neighbor_pose, g, h)
                    neighbor.parent = current
                    
                    # If neighbor is in open list with higher f, skip
                    if any(n.pos == neighbor_pose and n.f <= neighbor.f for n in open_list):
                        continue
...
'''
    ...
    start = (-0.1, -0.0, 0.1, 0 ,math.pi,0)
    goal = (-0.1, 0.7, 0.1, 0 ,math.pi,0)
    ...
    for p in path:
        ...
        q = quaternion_from_euler(p[3],p[4],p[5])
        T_desired[:3, :3] = quaternion_matrix(q)[:3, :3]
