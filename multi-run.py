...
class RobotMover(Node):
    ...
    def is_in_obstacle(self, pos , robot_type, orientation):
        x, y, z = pos
    ...
    # 3D A* Path Planning
    def a_star_3d(self, start, goal, orientation, robot_type, grid_size,  step_size=0.2):
        ...
        while open_list:
            ...
            for direction in directions:
                ...
                if (-2 <= x <= grid_size[0] and -2 <= y <= grid_size[1] and -2 <= z <= grid_size[2] and
                    ...
                    not self.is_in_obstacle(neighbor_pos , robot_type,orientation) and neighbor_pos not in closed_set):
    ...
    def follow_path(self, robot_type: str, path, socket_conn, orientation, delay=0.4):
        if not path:
            self.get_logger().error(f"No path provided for {robot_type}")
            return
        roll, pitch, yaw = orientation
        for p in path:
            T_desired = np.eye(4)
            ...
            solutions = self.inverse_kinematics(T_desired, robot_type)
            if solutions:
                # Select first solution (could improve with best solution selection)
                joint_angles = solutions[0]
                if not self.is_links_colliding_with_obs(joint_angles, robot_type):
                    success = self.move_real_robot(robot_type, joint_angles, socket_conn)
                    if success:
                        self.get_logger().info(f"{robot_type} moved to {p}")
                        time.sleep(delay)  # Delay to synchronize movement
                    else:
                        self.get_logger().warn(f"Failed to move {robot_type} to {p} due to collision or socket error")
                        break
                else:
                    self.get_logger().warn(f"Collision detected for {robot_type} at {p}")
                    break
            else:
                self.get_logger().warn(f"No IK solution for {robot_type} at {p}")
                break
    ...
    def connect_to_unity(self ):
        try:
            ...
            s.connect(("192.168.1.208", 12345))
        except ConnectionRefusedError as e:
            print("Connection failed:", e)

        return s

    def send_to_unity(self, s, joint_angles):
        angle_str = ",".join(map(str, joint_angles))  # e.g., "30,45,-20,0,10,-15"
        s.sendall(angle_str.encode())
...
'''
    ...
    s = node.connect_to_unity()
    node.send_to_unity(s, [180,-0, 0, 0,180,180])
    node.send_to_unity(s, [0,-0, 0, 0,180,180])
    time.sleep(2)
    node.send_to_unity(s, [180,-0, 0, 0,180,180])
    print(s)
    ...
    node.move_robot('ur3', [math.pi/2, -0, 0, 0, 0, 0.0])
    position, orientation = node.get_end_effector_pose('ur3')
    print(position)
    print(orientation)
    ...
    ur10e_start = (-0.1, 0.0, 0.1)
    ur10e_goal = (-0.1, 0.7, 0.1)
    ur3_start = (-0.23825000144901168, 0.009249997524958855, 0.31020000000000003)
    ur3_goal = (-0.14559999752495886, 0.5867499968954117, 0.06655000000000003)
    ur10e_orientation = [0.0, math.pi, 0.0]  # Same as original
    ur3_orientation = [1.5707963267948966, -0.0, 1.57079632]  # Different orientation for UR3
    # Define obstacles
    node.obstacles = [
        #{'center': (0.0, 0.4, 0.1), 'size': (0.2, 0.1, 0.2)},
        #{'center': (0.0, -0.4, 0.1), 'size': (0.2, 0.1, 0.2)},
    ]

    # Check start and goal positions
    if node.is_in_obstacle(ur10e_start, 'ur10e',ur10e_orientation):
        print("UR10e start is inside an obstacle!")
        return
    if node.is_in_obstacle(ur10e_goal, 'ur10e',ur10e_orientation):
        print("UR10e goal is inside an obstacle!")
        return
    if node.is_in_obstacle(ur3_start, 'ur3',ur3_orientation):
        print("UR3 start is inside an obstacle!")
        return
    if node.is_in_obstacle(ur3_goal, 'ur3',ur3_orientation):
        print("UR3 goal is inside an obstacle!")
        return

    # Plan paths
    ur3_path = node.a_star_3d(ur3_start, ur3_goal,ur3_orientation, 'ur3', grid_size, step_size)

    ur10e_path = node.a_star_3d(ur10e_start, ur10e_goal,ur10e_orientation, 'ur10e', grid_size, step_size)
    
    if ur3_path:
        print(f"UR3 path found with {len(ur3_path)} steps")
        node.visualize_path(ur3_start, ur3_goal, ur3_path, grid_size)
    else:
        print("No path found for UR3!")
        node.visualize_path(ur3_start, ur3_goal, None, grid_size)
        return
        
    # Visualize paths
    if ur10e_path:
        print(f"UR10e path found with {len(ur10e_path)} steps")
        node.visualize_path(ur10e_start, ur10e_goal, ur10e_path, grid_size)
    else:
        print("No path found for UR10e!")
        node.visualize_path(ur10e_start, ur10e_goal, None, grid_size)
        return
    ...
    # Connect to robots
    s_ur10e, s_ur3 = node.connect_to_robots(ROBOT_IP_UR10e, ROBOT_IP_UR3, PORT)
    if s_ur10e is None or s_ur3 is None:
        print("Failed to connect to robots, exiting")
        node.destroy_node()
        rclpy.shutdown()
        return

    # Move to initial positions
    node.move_robot('ur10e', [0.0, -math.pi/4, math.pi/4, 0.0, 0.0, 0.0])
    node.move_robot('ur3', [0.0, -math.pi/4, 0.0, 0.0, 0.0, 0.0])
    if node.move_real_robot('ur10e', [0.0, -math.pi/4, math.pi/4, 0.0, 0.0, 0.0], s_ur10e) and \
       node.move_real_robot('ur3', [0.0, -math.pi/4, 0.0, 0.0, 0.0, 0.0], s_ur3):
        print("Moved both robots to initial positions")
    else:
        print("Failed to move robots to initial positions")
        return

    # Execute paths in parallel
    ur10e_thread = threading.Thread(
        target=node.follow_path,
        args=('ur10e', ur10e_path, s_ur10e, 0.4)
    )
    ur3_thread = threading.Thread(
        target=node.follow_path,
        args=('ur3', ur3_path, s_ur3, 0.4)
    )
