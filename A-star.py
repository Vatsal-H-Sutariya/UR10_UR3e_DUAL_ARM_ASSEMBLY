...
class RobotMover(Node):
    ...
    def a_star_3d(self, start, goal, orientation, robot_type, grid_size,  step_size=0.2):
        ...
        directions = [(0, 0, step_size),(step_size, 0, 0), (-step_size, 0, 0), 
...
def main(args=None):
    ...
    T_desired = np.eye(4)
    T_desired[:3, 3] = [-0.40,-0.20,0.15]
    # Convert RPY to rotation matrix
    q = quaternion_from_euler(-1.611398378615906, -0.02483693000503847, 3.0828932839426617)
    T_desired[:3, :3] = quaternion_matrix(q)[:3, :3]
    ...
    node.move_real_robot('ur3', solutions[5], s_UR10e, s_UR3)
    print("done")
    '''

    
    #s_UR10e, s_UR3 =node.connect_to_robots(ROBOT_IP_UR10e,ROBOT_IP_UR3,PORT)
    #node.move_real_robot('ur3', [0.49,-0.36, 0.506, -3.24,-2.12,0], s_UR10e, s_UR3)
    ...
    step_size = 0.05 # Move 0.2 meters per step
    
    # Define start and goal points
    start = (-0.25, -0.20, 0.15)
    goal = (-0.55, -0.25, 0.15)
    
    # Define obstacles as variable-size cubes: {'center': (x, y, z), 'size': edge_length}
    node.obstacles = [
        {'center': (-0.40,-0.20, 0.18), 'size': (0.05, 0.05, 0.35)},  
        
        {'center': (0,0, -0.2), 'size': (1.945, 1.245, 0.4)},
        
        {'center': (1.945/2,0, 0), 'size': (0.05, 1.245,2)},
        {'center': (0,1.245/2, 0), 'size': (1.945, 0.01,2)},


         
    ]
    
    #node.move_real_robot('ur3', [0.9,-math.pi/2, 0, -math.pi,-math.pi/2,0], s_UR10e, s_UR3)
    
    # Ensure start and goal are not in obstacles
    if node.is_in_obstacle(start , 'ur3', [-1.611398378615906, -0.02483693000503847, 3.0828932839426617]
) :
        print("Start   is inside an obstacle!")
        return
    if node.is_in_obstacle(goal , 'ur3',[-1.611398378615906, -0.02483693000503847, 3.0828932839426617]
) :
        print("  goal is inside an obstacle!")
        return
     
    # Run A* algorithm
    path = node.a_star_3d(start, goal,[-1.611398378615906, -0.02483693000503847, 3.0828932839426617]
, 'ur3',grid_size , step_size)
